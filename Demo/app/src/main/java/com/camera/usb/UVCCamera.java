...
import com.camera.usb.USBMonitor.UsbControlBlock;
...
public class UVCCamera {
	...
	public static final int DEFAULT_PREVIEW_WIDTH = 3040;
	public static final int DEFAULT_PREVIEW_HEIGHT = 1520;
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
		...
		int rt = nativeExtensionOperateInitialize(mNativePtr);


		Log.v(TAG, "EasymovieTexture nativeExtensionOperateInitialize:" + rt);
    }
	/**
	 * set status callback
	 * @param callback
    ...
    public synchronized void close() {
    	stopPreview();
    	if (mNativePtr != 0) {
			nativeExtensionOperateRelease(mNativePtr);
...
//    		mNativePtr = 0;
    ...
    public void setFrameCallback(final IFrameCallback callback, final int pixelFormat) {
    	if (mNativePtr != 0) {
			if (DEBUG) Log.v(TAG, "setFrameCallback");
...
// 			   Log.d(TAG, "setZoom:zoom=" + zoom + " ,value=" + z);
	...
	public  String getCameraParams() throws JSONException {
		updateCameraParams();
		String supportsize = getSupportedSize();


		JSONObject json=new JSONObject();
		JSONArray jsonMembers = new JSONArray();
		JSONObject member1 = new JSONObject();
		member1.put("BrightnessMin", mBrightnessMin);
		member1.put("BrightnessMax", mBrightnessMax);
		member1.put("BrightnessDef", mBrightnessDef);


		member1.put("WhiteBlanceMin", mWhiteBlanceMin);
		member1.put("WhiteBlanceMax", mWhiteBlanceMax);
		member1.put("WhiteBlanceDef", mWhiteBlanceDef);

		jsonMembers.put(member1);

		json.put("uvcCamera", jsonMembers);
...
//**********************************************************************
	...
	public int ExtensionOperateSPIRead(final int address, final byte[] data,final int length) {

		return nativeExtensionOperateSPIRead(mNativePtr,address,data,length);
	}

	public int ExtensionOperateSPIWrite(final int address,final  byte[] data,final int length)
	{
		return nativeExtensionOperateSPIWrite(mNativePtr,address,data,length);
	}
    ...
    private static final native int nativeSetAnalogVideoLockState(final long id_camera, final int state);
    private static final native int nativeGetAnalogVideoLockState(final long id_camera);
	private static final native int nativeExtensionOperateInitialize(final long id_camera);
	private static final native int nativeExtensionOperateRelease(final long id_camera);
	private static final native int nativeExtensionOperateSPIRead(final long id_camera, final int start_address, byte[] data,final int length);
	private static final native int nativeExtensionOperateSPIWrite(final long id_camera, final int start_address, byte[] data,final int length);
	private static final native int nativeExtensionOperateI2CRead(final long id_camera, final int slave, final int address, short[] data,final int length);
	private static final native int nativeExtensionOperateI2CWrite(final long id_camera, final int slave, final int address, short[] data,final int length);
